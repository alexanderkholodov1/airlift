...
const PurificationResource := preload("res://scripts/purification/purification_resource.gd")
...
var _limbo_enemy_resolution: String = ""
...
func ingest_game_signal(signal_name: String, payload: Dictionary = {}) -> void:
	...
	match signal_name:
		...
		"boss_hit_by_brick":
			var hits := int(payload.get("hits", 1))
			register_attack_pacifist(clampf(0.35 + float(hits) * 0.15, 0.35, 2.0))
		"boss_defeated_violent":
			register_attack_pacifist(2.2)
		"boss_unplugged_cable":
			var cables := int(payload.get("count", 1))
		...
		"limbo_enemy_spared":
			_register_limbo_enemy_resolution("spared")
		"ignored_npc_favor":
...
func get_statistics() -> Dictionary:
	...
	var percentages := _build_sin_percentages(metrics)

	var weighted_impurity := (
		float(percentages.get("ira", 0.0)) * 0.33
		+ float(percentages.get("pereza", 0.0)) * 0.22
		+ float(percentages.get("gula", 0.0)) * 0.20
		+ float(percentages.get("soberbia", 0.0)) * 0.25
	)

	var prob_sin := clampf(weighted_impurity, 0.0, 100.0)
	var prob_bondad := clampf(100.0 - prob_sin, 0.0, 100.0)

	return {
		"probabilidad_bondad": prob_bondad,
		"probabilidad_pecado": prob_sin,
		"porcentajes_pecados": percentages,
		"valores_crudos": metrics,
	}
...
func reset_all() -> void:
	...
	history.clear()
	_brick_hoard_time = 0.0
	_limbo_enemy_resolution = ""
	_save_data()
	_emit_all()


func _register_limbo_enemy_resolution(resolution: String) -> void:
	# Solo contamos la decision del enemigo de Limbo una vez por partida.
	if _limbo_enemy_resolution != "":
		return

	_limbo_enemy_resolution = resolution
	if resolution == "killed":
		register_attack_pacifist(1.8)
	elif resolution == "spared":
		register_spare_pacifist(1.8)
...
func _build_sin_percentages(metrics: Dictionary) -> Dictionary:
	var ira := maxf(0.0, float(metrics.get("ira", 0.0)))
	var pereza := maxf(0.0, float(metrics.get("pereza", 0.0)))
	var gula := maxf(0.0, float(metrics.get("gula", 0.0)))
	var soberbia := maxf(0.0, float(metrics.get("soberbia", 0.0)))

	var total := ira + pereza + gula + soberbia
	if total <= 0.001:
		return {
			"ira": 0.0,
			"pereza": 0.0,
	...
	return {
		"ira": (ira / total) * 100.0,
		"pereza": (pereza / total) * 100.0,
		"gula": (gula / total) * 100.0,
		"soberbia": (soberbia / total) * 100.0,
	}
